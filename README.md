...
- `prism-ml/Ternary-Bonsai-2-27B-gguf`：本地 AI 导演，负责全片概念、叙事弧、视觉母题和分段剪辑策略。三值量化 27B，262K 上下文，经 `llama-server` 提供服务；权重默认从魔搭（ModelScope）下载；
...
默认 `auto` 模式优先从 ModelScope（魔搭社区）下载，适合中国大陆网络；某个仓库在魔搭不存在时才回退到 Hugging Face。导演的 GGUF 权重同样默认走魔搭，并且只下 `director_gguf_file` 指定的那一个量化文件——整仓快照要 13GB，而实际只需要其中一个。人声分离检查点来自 `audio-separator` 自己的目录，走单独路径，不受 `--source` 影响；未安装 `separation` extra 时会显示"跳过"并说明缺哪个 extra，其余模型照常下载。
...
| `image_composites` | `true` | 多图镜头的分屏、斜切、三联、主副网格和节拍蒙太奇 | 保持 `true` |
...
导演早期有一条 in-process `transformers` 路径（Spark-X2.5），现在**已整体移除**：远程代码兼容层、Accelerate 显存预算与卸载、多模态联系表都不在代码里了。随之失效的配置项是 `director_engine`、`director_backend`、`director_gpu_memory_gb`、`director_cpu_memory_gb`、`director_offload`、`director_contact_sheet_assets`——留在旧 `project.toml` 里不会报错（多余键被忽略），但什么也不做，建议删掉。
...
字幕默认**不加任何半透明底衬（scrim）**——那会让字周围整片变灰，是"浮在画面上"的另一种写法。两条机制取而代之：

- **逐行自适应描边**：每行按它落点区域的平均亮度决定描边宽度，在 `subtitle_outline`（暗背景基线）与 `subtitle_max_outline`（纯白背景上限）之间线性插值。亮沙滩加粗、夜景保持发丝边，这是亮背景可读性的主杠杆。测量区域在自由版式下是该行各分片的包围盒、band 版式下是底部字幕带；纯 CPU、无模型、确定性可复现（视频每个素材只抽一帧并缓存）。
- **局部压暗**：仅当某行背景亮度 ≥ 0.66 时启用。复用镂空的遮罩结构，把"提亮"换成"压暗"，用 `enable='between(t,行起点,行终点)+…'` 把字形区域**只在该行在屏窗口内**压暗——某行只在它自己的时间窗可见，门控后等价于只压该行字形。没有任何行需要它时，直接走单条 `ass` 滤镜，**默认帧无 scrim**。

配套地，**字幕静态基色唯一**：所有预设的静态 `\1c` 基线覆盖被移除，基线色统一为 `subtitle_highlight_color`；只有由颜色定义的效果（`rainbow`、`spotlight`）才在 `\t(...)` 动画里改色。`<0.6s` 的歌词行会并入相邻行、`<0.6s` 的分片并回整句，不再出现一闪而过的独立事件。像素级验证见 `scripts/subtitle_legibility_probe.py`。
...
- 铺垫型，用于主歌、前奏和尾奏：`cinematic_depth` 克制的景深推拉、`focus_pull` **同一张全出血帧的机内对焦**缓推（见下）；
...
两种复合动效按约十二分之一的概率落在单图镜头上——`parallax` 已**整体移除**，因为它的定义（同图分成模糊背景与清晰前景两个平面）正是"同源双尺度副本"本身：

- `film_bars`：叠加 2.35:1 上下遮幅，画面本身不裁切；
- `iris`：圆形遮罩从中心一点开到全画幅，像锁孔逐渐打开。

`focus_pull` 现在读作**对焦**，属于清晰度轴而不是取景轴：同一张全出血帧 `split` 成清晰与虚化两份（**同尺度**，因此不构成要禁的双尺度副本），用 `blend` 的 `all_expr` 按时间 `T` 交叉淡化，配合原有的小幅推进——画面在镜头内由虚到实。`gblur` 的 `sigma` 没有内联表达式选项，所以时间线走 `blend` 而不是 `sendcmd`。

多图合成。**每一种都把画面切成互不重叠的区域，或者一次只显示一张。** 没有任何一种会把两张图叠在同一片像素上——两张图争同一块地方，谁也没法保持可读，画面只会读成一片脏，而不是读成设计：

| 版式 | 张数 | 画面怎么分 |
| --- | --- | --- |
| `split_screen` | 2 | 等宽双栏 |
| `hero_split` | 2 | 2:1 主副双栏，宽的一侧是镜头、窄的一侧是同一时刻的第二眼 |
| `diagonal_split` | 2 | 斜线硬边分割，四种斜向按镜头轮换 |
| `triptych` | 3 | 三联竖排 |
| `hero_grid` | 3 | 左大 + 右侧上下双堆叠 |
| `beat_montage` | 2–4 | 镜头内部按音乐节拍依次切换，一次只显示一张 |

曾经有两个把画面叠起来的合成效果，`photo_stack`（卡片压在另一张满屏底图上）和 `double_exposure`（两张满屏图按银幕混合）——它们已经整体删除，因为"两张素材叠加在同一画面"正是画面发脏的来源。`tests/test_renderer.py` 里那条 `test_a_composite_never_paints_one_picture_over_another` 就是钉住这件事的：滤镜图里不许再出现 `blend=`、`rotate=` 和虚化底。

**分割出来的每一格都全出血，缝隙是画在接缝之上的一条半透明发丝线，不进宽度预算。** 单图镜头走**同一条**全出血路径（`_fill_frame`：`increase` 缩放 + 按 `focus_point` 裁切），与视频分支共用同一套主体感知裁切。曾经的单图处理（`_adapt_image`）只服务"填不满画面的图片"：按 92% 缩放居中、周围用**同一张图的虚化副本**补满。它在合成里错两次——每张图各自带来一片虚化底，一帧里两个背景互相竞争；而且同一张图以两个尺度出现两遍；并排时"一格留边、一格全出血"还会让两边取景和曝光不一致，读起来像两张恰好挨着的图。所以 `_adapt_image`/`_adapt_image_layers` 已删除，单图与合成格一律全出血。

缝隙必须画在画面上、而不是从宽度里挖出来：挖出来的话这组画面就比画布窄几个像素，最后的归一化会把整个镜头放大再用黑边补上，于是每一个分屏镜头左右都多出一条黑边——滤镜图本身完全合法，镜头也正常渲染，别的地方谁也看不见。`test_the_panels_of_a_division_add_up_to_the_canvas` 盯的就是这条，它要求各格宽度之和恰好等于画布宽度。

`diagonal_split` 的分割线是一张**半分辨率**的 `geq` 蒙版放大到全画幅。用 `iris` 那张八分之一分辨率的做法会把直线量化到八像素栅格上，放大后是一眼可见的锯齿；半分辨率下双线性边缘只有两像素的柔度，是接缝而不是阶梯，而蒙版本质上仍是一张矩形，开销可以忽略。蒙版只决定像素**归谁**，不做混合：`test_a_diagonal_cut_hands_each_pixel_to_one_picture_or_the_other` 用两块纯色渲染一帧，数出既不像这张也不像那张的像素——超过 3% 就算两张图被平均到了一起。

`beat_montage` 是**唯一有意保留留边**的合成效果：它一次只显示一张图，不存在两片虚化底互相竞争、也不存在同一张图两个尺度并存——正是这两点让"同时显示"的那几个效果出问题。而且每帧内缩也让蒙太奇读起来像一串照片，而不是几张整帧之间的硬切。它曾经用**同一张图的虚化副本**铺那圈留边，那是同源双尺度副本、也是画面里最脏的一层；现在换成该镜头主导色的**平铺色底**（`_matte_frame`）——内缩的读法价值保留，脏的那层去掉，帧内加框元素仍 ≤1。测试里专门写了一条断言把"它是有意的"固定下来，免得下次有人看到不一致又去"统一"。
...
多图辅助素材来自同一句歌词的视觉语义排序，同时受质量、色彩连续性、重复使用和分辨率惩罚约束。默认仅约 24% 图片镜头使用多图组合，副歌和 AI 导演标记的冲击段会提高概率，呼吸段会降低概率，避免整支 MV 变成模板化电子相册。`image_composites = false` 可只保留单图景深和方向性运镜。想看到更多多图镜头就抬 `image_composite_ratio`；`max_composite_images` 是单个镜头的素材上限，三联版式要它不低于 3。

**门控、版式轮转和运镜轮转各用各的计数器，三者回答的是三个不同问题**：

- **门控按镜头序号推进。** 这一条踩过坑：门控和运镜轮转曾经共用"已选出的单图镜头数"，而那个计数器**只在门控拒绝时**才前进——于是门控自我强化，一个镜头变成多图之后就把决定它的那个计数器冻住了，此后永远返回同一个答案。实测同一首歌在 `image_composite_ratio` 设成 0.24、0.5、1.0 时输出完全一样，**每个图片镜头都是多图合成**，配置项形同虚设。`test_the_composite_gate_follows_the_configured_share` 用两个比例跑同一首歌来钉住它。
- **版式轮转也按镜头序号推进。** 按镜头序号取模，跨度够长的段落里每个槽位都会轮到；早先挂在同一个被冻住的计数器上时，整首歌只会出现轮转表的第一项，新加的版式等于没加。
- **运镜轮转按"已选出的单图镜头数"推进**，而不是按镜头总序号。两者若共用序号，被合成吃掉的那些序号对应的轮转槽位就永远轮不到——实测同一首歌只用到 13 种运镜，改成独立计数后用满 21 种。
...
`avoid_asset_repeats = true`（默认）时复用规则分成两条：**非母题零复用，母题按段落复现**。

- **非母题**：规划器把非母题素材按"已出现次数"分层，每个镜头都从出现次数最少的非母题里选，因此只要池中还有没出现过的非母题，就绝不选已用过的——素材够用时全片无"陌生人"复用，素材不足时复用次数在所有非母题之间尽量均分，而不是反复用语义最匹配的那几张。相邻镜头的复用额外扣分，同一句重复歌词也不会用同一张画面。
- **母题**：AI 导演标出的视觉母题（`motif_asset_ids`，≤5 个）是**有意**复现的例外——它们在主循环之前就按段落轮转排进保留位（每个母题目标次数 `max(3, round(镜头数 × 10% / 母题数))`），成片因此有一小批"认得出"的画面，而不是几百张陌生人脸。母题的复现占比封顶约 30%：短歌装不下 3 次复现时**减少母题个数**，而不是把母题硬凑上去。复现次数写入 `plan.json` 的 `motif_appearances`（`{素材 id: 出现次数}`），`Shot.is_motif` 标出每个保留位。

多图合成按"富余素材"计费：只有当**非母题**素材在满足剩余镜头所需之后还有剩余时，才允许把它用作多图版式的辅助图层，否则该镜头退回单图运镜（母题保留位不算进这份富余，它们已经被预订）。素材非常紧张时（例如 3 张图配 40 个镜头）多图合成仍然保留，因为此时复用已经无法避免。

`avoid_asset_repeats = false` 会放弃"非母题零复用"，退回旧的"按得分自由复用"行为。
...
frame_samples = 3
director_enabled = true
# 纯 CPU 机器可以关掉导演（director_enabled = false），或把 -ngl 设为 0 走 CPU 推理，
# 但 27B 三值模型在 CPU 上很慢，功能验证建议直接用规则导演。
director_prompt_tokens = 8192
director_llama_args = ["-ngl", "0", "-fa", "on", "-c", "8192"]
```
...
> `camera_motion` 已**整体移除**：`MediaAsset`、`Shot`、`plan.json` 与导演 brief 中都不再存在该字段。旧 sidecar 若仍写着它，本次运行会以 `deprecated-key` 记一条 WARNING 并写入 `config_audit`，其值被忽略。视频的运动强度改为纯能量口径。
...
专门看多图版式的那张接触表（四种源图形状：16:9、3:4、1:1、9:16，每种版式渲染一张，**改 `_composite_graph` 之后先跑它**）：

```powershell
uv run python scripts/composite_probe.py
```
...
beatforge/models/ai_director.py       本地 AI 导演协议、提示词裁剪与校验
